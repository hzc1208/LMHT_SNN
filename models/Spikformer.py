...
__all__ = ['spikformer_cifar']

# code from https://github.com/ZK-Zhou/spikformer/tree/main
...
class SPS(nn.Module):
    def __init__(self, T, img_size_h=128, img_size_w=128, in_channels=3, embed_dims=256):
        super().__init__()
        self.T = T
...
class Spikformer(nn.Module):
    def __init__(self, T, img_size_h, img_size_w, in_channels, num_classes, embed_dims, num_heads, mlp_ratios, depths, sr_ratios, qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.):
        super().__init__()
        self.T = T
        ...
        patch_embed = SPS(T=T, img_size_h=img_size_h,
    ...
    def forward(self, x):
        ...
        T, B, C = x.shape
        x = self.head(x.view(-1, C)).view(T, B, -1)
        return x


@register_model
def spikformer_cifar(T, cls, **kwargs):
    model = Spikformer(
        T=T, img_size_h=32, img_size_w=32, in_channels=3, num_classes=cls,
        embed_dims=384, num_heads=12, mlp_ratios=4,
        depths=4, sr_ratios=1, qkv_bias=False,
        **kwargs
    )
